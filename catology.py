...
def categorize(save = True):
    ...
    if(save):
        df_categorized.to_excel('xlsx/mainCategorized.xlsx', index=False)
        os.startfile('xlsx/mainCategorized.xlsx')
...
def numerical_transform(to_categorize = False, save = True):
    ...
    if(save):
        df.to_excel('xlsx/mainNumsOnly.xlsx', index=False)
        os.startfile(os.path.join('xlsx', 'mainNumsOnly.xlsx'))
    return df
